...
function bulk_at(f, scheme)
    ...
    f > 0 && add_phase!(rve, :V, Ellipsoid(1.0),
...
plot!(p, fs, [bulk_at(f, AsymmetricSelfConsistent(; abstol = 1.0e-10, maxiters = 200))
              for f in fs]; label = "ASC", color = :purple, lw = 2)
plot!(p, fs, [bulk_at(f, DifferentialScheme(; nsteps = 200))
              for f in fs]; label = "Differential", color = :orange, lw = 2)
