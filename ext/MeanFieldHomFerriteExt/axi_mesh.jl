...
"""
    _build_gmsh_axi_model(a, a_core, d, R, h_in, h_out)
...
function _build_gmsh_axi_model(
        a::Float64, a_core::Float64, d::Float64, R::Float64,
...
"""
    _axi_grid(incl) -> Ferrite.Grid{2}

Two-dimensional grid of the meridian half-plane of `incl`, with cell sets
`"core"`, `"shell"`, `"matrix"` and facet sets `"outer"`, `"axis"`.

The first coordinate of a node is the cylindrical radius `ρ`, the second the
axial coordinate `z`.
"""
function _axi_grid(incl::MeanFieldHom.FEExcenteredSphere)
    a = Float64(incl.a)
