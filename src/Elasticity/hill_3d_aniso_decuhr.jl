...
function _hill_3d_aniso_decuhr(ell::Ellipsoid{3}, C₀;
                                abstol::Float64 = 1e-8,
                                reltol::Float64 = 1e-6,
    ...
    function integrand_at(z, φ)
        ...
        for I in 1:6
            ...
            for J in I:6
                k, l = voigt_ij[J]
                γ = T(0.25) * (ζ[i]*(iK[j,k]*ζ[l] + iK[j,l]*ζ[k]) +
    ...
    P_vals, _ = QuadGK.quadgk(0.0, 1.0;
                              atol = abstol,
                              rtol = reltol,
                              maxevals = maxiters) do z
        inner, _ = QuadGK.quadgk(0.0, 2π;
                                 atol = abstol / 10,
                                 rtol = reltol) do φ
            integrand_at(z, φ)
        end
