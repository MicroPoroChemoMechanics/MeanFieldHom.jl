...
@testset "Crack regression cases" begin
    ...
    @testset "Analytical COD — isotropic matrix" begin
        ...
        @test B_penny[3,3] ≈ Bnn_expect rtol=1e-10
        @test B_penny[1,1] ≈ Bll_expect rtol=1e-10
        @test B_penny[2,2] ≈ Bll_expect rtol=1e-10

        # Ribbon analytical values match closed-form
        r = RibbonCrack(1.0)
        B_r = cod_tensor(r, C₀)
        χ = π * (1 - ν^2) / E
        @test B_r[2,2] ≈ χ                         rtol = 1e-10
        @test B_r[3,3] ≈ χ                         rtol = 1e-10
        @test B_r[1,1] ≈ χ / (1 - ν)               rtol = 1e-10
    end
    ...
    @testset "H ↔ B conversion (diagonal slot only)" begin
        ...
        @test B_back[3,3] ≈ B[3,3] atol=1e-12
    end
    ...
    @testset "Compliance contribution — ellipse vs ribbon pre-factors" begin
        ...
        for i=1:3, j=1:3, k=1:3, l=1:3
            @test ΔS_p[i,j,k,l] ≈ expected_p[i,j,k,l] rtol=1e-12
        end
        ...
        for i=1:3, j=1:3, k=1:3, l=1:3
            @test ΔS_r[i,j,k,l] ≈ expected_r[i,j,k,l] rtol=1e-12
        end
    end
    ...
    @testset "SIF — ribbon analytical" begin
        ...
        @test norm(𝐊) ≈ √(π * b) rtol = 1e-10
        @test Kᴵ  ≈ √(π * b) rtol = 1e-10
        @test abs(Kᴵᴵ)  < 1e-12
        @test abs(Kᴵᴵᴵ) < 1e-12
    end
    ...
    @testset "DIF — ellipse isotropic penny under uniaxial tension" begin
        ...
        @test d[3] ≈ 16 * (1 - ν^2) / (3 * π * E) rtol = 1e-10
        @test abs(d[1]) < 1e-12
        @test abs(d[2]) < 1e-12
    end
