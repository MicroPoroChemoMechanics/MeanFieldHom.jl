...
@testset "Hill tensor regression cases" begin
    ...
    @testset "Auxiliary tensors — sphere 3D" begin
        ...
        @test IA[1,1] ≈ 1/3  atol=1e-12
        @test IA[2,2] ≈ 1/3  atol=1e-12
        @test IA[3,3] ≈ 1/3  atol=1e-12
        @test IA[1,2] ≈ 0.0  atol=1e-12
        sum_IA = IA[1,1] + IA[2,2] + IA[3,3]
        @test sum_IA ≈ 1.0  atol=1e-12

        @test VA[1,1,1,1] ≈ 1/3  atol=1e-12
        @test VA[1,2,1,2] ≈ 1/6  atol=1e-12
        @test VA[1,1,2,2] ≈ 0.0  atol=1e-12

        @test UA[1,1,1,1] ≈ 1/5  atol=1e-12
        @test UA[1,1,2,2] ≈ 1/15  atol=1e-10
        @test UA[1,2,1,2] ≈ 1/15  atol=1e-10
    end

    @testset "Auxiliary tensors — prolate spheroid 3D" begin
        ell = Ellipsoid(2.0, 1.0, 1.0)
        IA = tens_IA(ell)
        @test IA[2,2] ≈ IA[3,3]  atol=1e-10
        @test IA[1,1] + IA[2,2] + IA[3,3] ≈ 1.0  atol=1e-10
        @test IA[1,1] < IA[2,2]
    end
    ...
    @testset "Auxiliary tensors — circle 2D" begin
        ...
        IA = tens_IA(ell)
        @test IA[1,1] ≈ 0.5  atol=1e-12
        @test IA[2,2] ≈ 0.5  atol=1e-12
        @test IA[1,1] + IA[2,2] ≈ 1.0  atol=1e-12
    end
    ...
    @testset "Hill 3D iso — sphere" begin
        ...
        @test P[1,1,1,1] ≈ P1111_expect  atol=1e-10

        @test P[1,1,2,2] ≈ P[2,2,1,1]  atol=1e-12
        @test P[1,2,1,2] ≈ P[2,1,1,2]  atol=1e-12
        @test P[1,1,1,1] ≈ P[2,2,2,2]  atol=1e-10
        @test P[1,1,1,1] ≈ P[3,3,3,3]  atol=1e-10
    end
    ...
    @testset "Hill 3D iso vs DECUHR — prolate spheroid" begin
        ...
        P_dcr = hill_tensor(ell, C_full; method=:decuhr, abstol=1e-8, reltol=1e-6)

        for i in 1:3, j in 1:3, k in 1:3, l in 1:3
            @test P_dcr[i,j,k,l] ≈ P_iso[i,j,k,l]  atol=5e-5
        end
    end
    ...
    @testset "Hill order-2 (conductivity) — sphere 3D iso" begin
        ...
        k_cond = K_iso3[1,1]
        @test P[1,1] ≈ 1/(3k_cond)  atol=1e-12
        @test P[2,2] ≈ 1/(3k_cond)  atol=1e-12
        @test P[3,3] ≈ 1/(3k_cond)  atol=1e-12
        @test P[1,2] ≈ 0.0           atol=1e-12
    end
    ...
    @testset "Hill order-2 (conductivity) — prolate spheroid 3D iso" begin
        ...
        k_cond = K_iso3[1,1]
        IA = tens_IA(ell)
        @test P[1,1] ≈ IA[1,1] / k_cond  atol=1e-10
        @test P[2,2] ≈ IA[2,2] / k_cond  atol=1e-10
        @test P[3,3] ≈ IA[3,3] / k_cond  atol=1e-10
        @test P[2,2] ≈ P[3,3]  atol=1e-10
    end
