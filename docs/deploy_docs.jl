...
#   julia --project=docs/ docs/deploy_docs.jl v0.8.0   # force tag
#
...
repo_remote = "git@codeberg-docs:MicroPoroChemoMechanics/MeanFieldHom.jl.git"
...
cd(pagesdir) do
    run(`git config user.name "MeanFieldHom Documenter"`)
    run(`git config user.email "docs@meanfieldhom.codeberg.page"`)
